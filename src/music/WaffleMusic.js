...
class WaffleMusic {
      ...
      this._getYTInfo(args)
        .then((info) => {
          if (!info) {
            ...
            parseInt(info.player_response.videoDetails.lengthSeconds) > 600
          ) {
            return Promise.resolve(
              "Aint nobody got time fo dat. That song is too damn long!"
            );
          }
  ...
  static _pause() {
    ...
    if (dispatcher && !dispatcher.paused) {
      ...
      const {
        title,
      } = this.musicQueue.peek().info.player_response.videoDetails;
      return wr.setResponse(`*paused ${title}*`);
    }
    return wr
      .setResponse(`Can't pause what's already paused, genius`)
      .setIsError(true);
  }
  ...
  static _unpause() {
    ...
    return wr
      .setResponse(`*can't unpause what's not paused, genius.*`)
      .setIsError(true);
  }
}
