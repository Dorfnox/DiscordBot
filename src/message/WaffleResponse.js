      ...
      setTimeout(() => {
        getSafe(
          () => {
            ...
            const errorLocale = this.errorLocale
              ? ` | ${this.errorLocale}`
              : "";
            const logMsg = `\n__MSG ${getSafe(() => msg.content || "", "")}`;
            const response = `\n__RES ${JSON.stringify(
              this.response.embed || this.response
            )}`;
            const logResponse =
              this.logResponseLimit > -1
                ? `${response.substr(0, this.logResponseLimit)}${
                    this.logResponseLimit < response.length
                      ? `... +${
                          response.length - this.logResponseLimit
                        } more chars`
                      : ""
                  }`
                : response;
            const logError = this.error
              ...
              : "";
            logger(
              `[${now} | ${username}${errorLocale}] ${logMsg}${logResponse}${logError}\n`
            );
          },
          null,
          (err) => console.error("WR LOG ERROR: ", err, "\n")
        );
      }, 100);
    }
    if (cb) {
